...
var init_http_exception = __esm({
  "../node_modules/hono/dist/http-exception.js"() {
    init_functionsRoutes_0_01603471412361346();
  }
});
...
var import_readability, app, HTML, src_default;
var init_src = __esm({
  "../src/index.ts"() {
    "use strict";
    init_functionsRoutes_0_01603471412361346();
...
<link rel=icon href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>\u{1F4C4}</text></svg>">
<style>
:root {
  --bg: #fafaf9;
  --card-bg: #fff;
...
function toggleReadingMode(el){
  ...
  el.textContent=readingMode?'\u{1F4D6} Exit Reading Mode':'\u{1F50D} Reading Mode';
...
// \u2500\u2500 Staged loading (#3) \u2500\u2500
const STAGES=[
  {label:'Fetching article...',icon:'\u{1F310}'},
  {label:'Extracting content...',icon:'\u{1F4C4}'},
...
function categorizeError(msg){
  if(msg.includes('403')||msg.includes('block'))return {emoji:'\u{1F512}',title:'Access Denied',hint:'This site blocks bots. Try a public article or paste text directly.'};
  if(msg.includes('404')||msg.includes('not found'))return {emoji:'\u{1F50D}',title:'Page Not Found',hint:'The URL does not point to a valid article. Check the link and try again.'};
  ...
  if(msg.includes('extract')||msg.includes('content'))return {emoji:'\u{1F4C4}',title:'Extraction Failed',hint:'We could not extract the article body. The page might be a video, image gallery, or PDF.'};
...
function showError(msg){
  ...
  out.innerHTML='<div class=error-card>'+
    ...
    '<div class=error-fallback>'+
      '<button onclick="showPasteTextArea()" style="font-size:.85rem;padding:8px 16px;margin-top:8px">\u{1F4DD} Paste text manually</button>'+
    '</div>'+
  '</div>';
}
  ...
  out.innerHTML=
    '<div class=result-card>'+
      ...
      '<div class=article-meta>'+
        '<span>\u{1F552} '+meta.minutes+' min read</span>'+
        '<span>\u{1F4CB} '+meta.words.toLocaleString()+' words</span>'+
        (hostname?'<span>\u{1F310} '+hostname+'</span>':'')+
      '</div>'+
      // Font controls (#2)
      ...
      '<div class=read-mode-bar>'+
        '<button class="read-mode-btn" onclick="toggleReadingMode(this)" style="width:auto">\u{1F50D} Reading Mode</button>'+
      '</div>'+
      // Summary card with gradient glow (#4)
      ...
      '<div class=actions>'+
        '<div class=dropdown>'+
          '<button class=dropdown-toggle onclick="event.stopPropagation();this.nextElementSibling.classList.toggle('open')">\u{1F4E5} Download <span style=font-size:.75rem>\u25BC</span></button>'+
          '<div class=dropdown-menu>'+
            '<button class=dropdown-item data-format=md><span>\u{1F4DD}</span> Markdown <span class=ext-info>~'+Math.round((d.textContent||'').length/1024)+' KB</span></button>'+
            '<button class=dropdown-item data-format=txt><span>\u{1F4C4}</span> Plain Text</button>'+
            '<button class=dropdown-item data-format=html><span>\u{1F310}</span> HTML</button>'+
          '</div>'+
        '</div>'+
      '</div>'+
    '</div>';
    ...
    app.post("/api/extract", async (c) => {
      ...
      try {
        ...
        if (article) {
          ...
          textContent2 = article.textContent || "";
        }
      } catch {
      }
      if (!title && !textContent2) {
        return c.json({ error: "Could not extract content from this page." }, 400);
      }
      let summary = "Summary unavailable";
      try {
        const promptText = textContent2.slice(0, 8e3);
      ...
      const result = { title, content, textContent: textContent2, summary, url };
      try {
...
var init_route = __esm({
  "[[route]].ts"() {
    ...
    init_src();
    onRequest = handle(src_default);
  }
});

// ../.wrangler/tmp/pages-iOpymw/functionsRoutes-0.01603471412361346.mjs
var routes;
var init_functionsRoutes_0_01603471412361346 = __esm({
  "../.wrangler/tmp/pages-iOpymw/functionsRoutes-0.01603471412361346.mjs"() {
    "use strict";
    init_route();
    routes = [
